...
router.get("/top10", async (req, res) => {
  try {
    ...
    // Dynamically calculate required ads based on ad_frequency
    const requestedAds = parseInt(ads) || 0;
    const adFrequency = await getAdFrequency();
    const adLimit = requestedAds > 0 ? Math.ceil(fixedLimit / adFrequency) : 0;
    ...
    manualRes.rows.forEach(row => {
      // Normalize categories to lowercase array for consistent matching
      const itemCategories = Array.isArray(row.category)
        ? row.category.map(c => c.toLowerCase())
        : [];

      manualOverrides[row.rank] = {
        ...row,
        normalizedCategories: itemCategories
      };
      manualNewsIds.add(row.news_id);
    });
    ...
    for (let rank = 1; rank <= fixedLimit; rank++) {
      const override = manualOverrides[rank];
      if (override) {
        // Check if the override news item belongs to the requested categories
        const matchesCategory = categories.includes("all") ||
          override.normalizedCategories.some(cat => categories.includes(cat));

        if (matchesCategory) {
          mergedList.push(override);
          continue; // Successfully used matching override
        }
      }

      // Fall back to algorithmic news if no matching override
      if (algoIndex < finalNews.length) {
        mergedList.push(finalNews[algoIndex]);
        algoIndex++;
      }
...
router.get("/feed", verifyToken, async (req, res) => {
  try {
    ...
    // Dynamically calculate required ads based on ad_frequency
    const adFrequency = await getAdFrequency();
    const reqAdsStr = String(ads).toLowerCase();
    const isAdsDisabled = reqAdsStr === '0' || reqAdsStr === 'false';
    const parsedAds = isAdsDisabled ? 0 : Math.ceil(parsedLimit / adFrequency);
    ...
    let adQuery = `
      ...
      LEFT JOIN (SELECT ad_id, COUNT(*) AS comment_count FROM comments GROUP BY ad_id) c ON a.ad_id = c.ad_id
      LEFT JOIN (SELECT ad_id, COUNT(*) AS share_count FROM shares GROUP BY ad_id) s ON a.ad_id = s.ad_id
      LEFT JOIN news_likes ul ON ul.ad_id = a.ad_id AND ul.user_id = $1
