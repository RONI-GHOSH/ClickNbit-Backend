...
router.get('/', async (req, res) => {
  try {
    ...
    if (target_category) {
      whereConditions.push(`$${queryParams.length + 1} = ANY(a.target_categories)`);
    ...
    // Add sorting
    const validSortColumns = ['created_at', 'priority_score', 'start_at', 'end_at'];
  ...
  try {
    ...
    const query = `
      INSERT INTO advertisements (
        ...
        $1, $2, $3, $4, $5, $6, $7, $8, $9, 
        ${geo_point ? 'ST_GeographyFromText($10)' : 'NULL'}, 
...
router.put('/:id', verifyAdmin, async (req, res) => {
  try {
    ...
    const checkResult = await pool.query('SELECT * FROM advertisements WHERE ad_id = $1', [id]);
...
router.post('/:id/events', async (req, res) => {
  try {
    ...
    // Validate required fields
    if (!event_type || !['view', 'click', 'like', 'share'].includes(event_type)) {
      return res.status(400).json({ 
        success: false, 
